...
local phases = {
	...
	["Start"] = {
		...
		init = function(manager, values)
			ThePlayer.HUD.tutorialtext:SetTutorialText(values.tutorialtext)
			ThePlayer.HUD.tutorialtext:FadeIn()
		...
		cleanup = function(manager, values)
			ThePlayer.HUD.tutorialtext:FadeOut()
			ThePlayer.HUD.batteryindicator:FadeIn()
		end,
	},
	["TeachToggle"] = {
		values = {
		...
		update = function(manager, values)
            local flashlight_ent = ThePlayer.FlashlightEnt()
            if flashlight_ent then
    ...
    ["TeachRoad"] = {
        ...
        init = function(manager, values)
            values.posx, values.posy, values.posz = ThePlayer:GetPosition()
            values.stoptime = GetTime() + values.length
        end,
        update = function(manager, values)
            local origin = Vector3(ThePlayer.homefirepit.Transform:GetWorldPosition())
            local dist_sq = distsq(origin, ThePlayer:GetPosition())
            ...
            if GetTime() > values.stoptime and not values.leftcamp then
                ThePlayer.components.talker:Say("I wonder if anyone's down that road.", 2.5, false)
                values.leftcamp = true
            end
        end,
        cleanup = function(manager, values)
	...
	["FirstCamper"] = {
		...
		init = function(manager, values)
            manager.inst:ListenForEvent("firstcamperactivated", function()
            	ThePlayer:PushEvent("change_breathing", {intensity = 2, duration=4})
		...
		update = function(manager, values)
			if manager.campers_encountered == 1 or manager.area.story == "Camp2_Task" then
                ThePlayer.components.scarymodmusic:SetBaseMusicLevel(TUNING.FIRST_MUSIC_INCREASE)
				manager:NextBeat()
	        end
		end,
		cleanup = function(manager, values)
    ...
    ["SecondCamper"] = {
        ...
        update = function(manager, values)
            if manager.campers_encountered == 2 or manager.area.story == "Camp2_to_3" then
                ThePlayer.components.scarymodmusic:SetBaseMusicLevel(TUNING.SECOND_MUSIC_INCREASE)
                manager:NextBeat()
            end
        end,
        cleanup = function(manager, values)
    ...
    ["StalkCamper"] = {
        ...
        init = function(manager, values)
        	ThePlayer:PushEvent("change_breathing", {intensity = 2, duration=-1})
        end,
        update = function(manager, values)
			...
			local dist_sq = distsq(manager.shamblers[1]:GetPosition(), ThePlayer:GetPosition())
        ...
        cleanup = function(manager, values)
        	ThePlayer:PushEvent("change_breathing", {intensity = 1, duration=-1})
        end,
    },
	["NoteWatch1"] = {
		values = {
		...
		init = function(manager, values)
            ...
            manager.inst:ListenForEvent("shownote", function(inst, data)
                if data.note == 3 then --3 is the "Eats the light, feeds in the dark" note
                	ThePlayer.SoundEmitter:PlaySound("scary_mod/music/hit_light_piano")
                end
            end)
            manager.inst:ListenForEvent("hidenote", function(inst, data)
                if data.note == 3 then --3 is the "Eats the light, feeds in the dark" note
					...
					manager.inst:DoTaskInTime(1.4, function()                 	
						ThePlayer.SoundEmitter:PlaySound("scary_mod/stuff/blowout")
					end)

                    manager.inst:DoTaskInTime(1.7, function() 
		               	--ThePlayer.SoundEmitter:PlaySound("scary_mod/stuff/tension")
                    	ThePlayer:PushEvent("change_default_breathing", {intensity = 2})
                        local player = ThePlayer
                        local x,y,z = player.Transform:GetWorldPosition()
	                        ...
	                        if flashlight_ent then
	                            ...
	                            manager.inst:DoTaskInTime(1.0, function() 
		                            if flicker.ison and flicker.cantoggle then
			                            ...
			                            ThePlayer.components.talker:Say("Shit! My flashlight!", 2, false)
		                                ThePlayer.SoundEmitter:PlaySound("scary_mod/stuff/flashlight_out")
		                            end
		                        end)
	                        end
	                    end)
	                    ThePlayer.HUD.externalblockmap = true
	                    ThePlayer:DoTaskInTime(5, function()
	                    	ThePlayer.HUD.externalblockmap = false
	                    end)
                        values.blewoutfire = true
                        values.shamblerspawntime = GetTime() + values.shamblerspawndelay
                        ThePlayer.components.scarymodmusic:SetBaseMusicLevel(TUNING.THIRD_MUSIC_INCREASE)
                    end)
                end
            end)
		end,
		update = function(manager, values)
			if values.blewoutfire and GetTime() > values.shamblerspawntime then
                ...
                if not values.spawnedshambler then
					...
					values.player_start_pos = ThePlayer:GetPosition()
				else
					local dist_sq = distsq(values.player_start_pos, ThePlayer:GetPosition())
        ...
        avoidedshambler = function(manager, values)
            ThePlayer:PushEvent("change_breathing", {intensity = 3, duration=6})
            values.finishedshambler = true
        end,
		cleanup = function(manager, values)
    ...
    ["ThirdCamper"] = {
        ...
        update = function(manager, values)
            if manager.campers_encountered == 3 or manager.area.story == "Camp4_Task" then
                --ThePlayer.components.scarymodmusic:SetBaseMusicLevel(TUNING.FOURTH_MUSIC_INCREASE)
                manager:NextBeat()
            end
        end,
        cleanup = function(manager, values)
	...
	["InvisibleStalking"] = {
		...
		update = function(manager, values)
			...
			if values.next_stalk < GetTime() then
				...
				local center = ThePlayer:GetPosition()
    ...
    ["Helipad"] = {
        ...
        init = function(manager, values)
        	...
        	local fulllight = function()
	            ...
	            --ThePlayer:PushEvent("spawn_shambler")
	        end
            ...
            manager.inst:ListenForEvent("generator_pull", function(inst,data)
            	...
            	if data.num_generator_pulls == 1 then
	            	...
	            	player:DoTaskInTime(1.2, function()
	            		ThePlayer.components.talker:Say("Dammit, start already!", 2, false)
	            		--manager:StartStaticRing()
	            	end)

	            	player:DoTaskInTime(1.7, function()
	            		--player.SoundEmitter:PlaySound("scary_mod/stuff/screetch_scream")
	            		ThePlayer:PushEvent("change_breathing", {intensity = 2, duration=20})
	            		ThePlayer.components.scarymodmusic:SetBaseMusicLevel(TUNING.FIFTH_MUSIC_INCREASE)
	            	end)
	            ...
	            elseif data.num_generator_pulls == 2 then
	            	...
	            	player:DoTaskInTime(1.2, function()
	            		ThePlayer.components.talker:Say("Come on, come on!", 2, false)
	            	end)
	            	...
	            	player:DoTaskInTime(1.5, function()
	            		...
	            		fulllight()
		            	player:DoTaskInTime(1.2, function()
		            		ThePlayer.components.talker:Say("All right!", 2, false)
		            		ThePlayer.components.scarymodmusic:SetBaseMusicLevel(TUNING.FIFTH_MUSIC_INCREASE + 0.05)
		            		ThePlayer:DoTaskInTime(0.5, function()
		            			ThePlayer:PushEvent("change_breathing", {intensity = 1, duration=20})
		            		end)
				            	...
				            	player:DoTaskInTime(8.5, function(player)
					                ...
					                player:ListenForEvent("flashlighttoggleon", function()
					                	...
					                	if not manager.gameover then
						                	...
						                	ThePlayer.HUD.batteryindicator:Hide()
										    local player = ThePlayer
...
function ScaryModEncounterManager:CheckShamblersForVisBlocking()
	if #self.shamblers > 0 then
		...
		if not CastToPosition(ThePlayer:GetPosition(), shambler:GetPosition(), 5, {"visblocker"}) then
...
function ScaryModEncounterManager:SpawnShamblerNearby(kind)
	...
	local facing = ThePlayer.Transform:GetRotation()
	local facingvec = Vector3(math.cos(facing*DEGREES),0,math.sin(facing*DEGREES))
	local position = ThePlayer:GetPosition()
...
function ScaryModEncounterManager:SpawnShamblerInfront(kind)
	...
	local angle = ThePlayer.Transform:GetRotation()
...
function ScaryModEncounterManager:StartStaticRing()
	...
	--ThePlayer.SoundEmitter:PlaySound("scary_mod/stuff/staticring","staticring")
end
...
function ScaryModEncounterManager:KillStaticRing()
	...
	--ThePlayer.SoundEmitter:KillSound("staticring")
end
...
function ScaryModEncounterManager:DoFinaleDeath(reason)
    ...
    ThePlayer:PushEvent("killallsounds")
	...
	ThePlayer.HUD.bloodover:KillOverlay()
	ThePlayer.HUD.batteryindicator:Hide()
	ThePlayer.components.characterbreathing:StopBreathing()
	ThePlayer:PushEvent("killgensound")
	ThePlayer.SoundEmitter:KillSound("heli")
	ThePlayer.SoundEmitter:KillAllSounds()
	ThePlayer.HUD:EndSequence()
...
function ScaryModEncounterManager:DoBeforeFinaleDeath(reason)
    ...
    ThePlayer.components.health:Kill()
    ThePlayer:PushEvent("killallsounds")
    local x,y,z = ThePlayer.Transform:GetWorldPosition()
    local fires = TheSim:FindEntities(x,y,z, 300, {"fx"})
    for i, f in pairs(fires) do
