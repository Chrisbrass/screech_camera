local LightBeam = Class(function(self, inst)
	...
	self.inst:ListenForEvent("finishedsearchingcontainer", function() 
		...
		self.transitioning = true
		self.timetransitioning = 0
		ThePlayer.components.scarymodencountermanager:SuggestShamblerLocation(ThePlayer:GetPosition(), self.forwardoffset + self.lightlength, ThePlayer.Transform:GetRotation())
	end, ThePlayer)
...
function LightBeam:OnUpdate(dt)
	local playerpos = self.inst:GetPosition()
	local playerfacing = ThePlayer.Transform:GetRotation()
